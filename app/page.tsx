...
import { DaisyField, SunGlow, Celestial, Laurel } from "./components/GoldenDecals";
import { Testimonials } from "./components/Testimonials";
import { ServiceCard } from "./components/ServiceCard";
import { featuredServices, browLashServices } from "./data/services";
...
export default async function Home() {
  ...
  return <>
    ...
    <section className="hero"><SunGlow /><Celestial /><DaisyField variant="hero" /><div className="shell hero-inner"><div className="hero-copy"><p className="eyebrow">Golden Esthetics · Licensed Esthetician</p><h1>{headline.first}{headline.second && <><br/><em>{headline.second}</em></>}</h1><p className="lede">{content.heroSupportingCopy}</p><div className="button-row"><a className="button button-primary" href="/find-my-facial">Try Sparrow Skin Match <span>✦</span></a><a className="button button-quiet" href="/services">Explore services</a></div><div className="trust-row"><span>Personalized care</span><span>Thoughtful treatments</span><span>A glow that feels like you</span></div></div><div className="hero-portrait"><div className="portrait-placeholder">
  <img
    src="/kinnley profile pic.jpg"
    alt="McKinnley of Golden Esthetics"
    className="portrait-image"
  />
</div><div className="portrait-note"><strong>Care, customized.</strong><br/>Every service begins with you.</div></div></div></section>
    <section className="about-preview shell"><div className="about-art"><img className="ge-seal" src="/golden-seal.jpg" alt="Golden Esthetics monogram" /><Laurel className="seal-laurel" /><i className="ge-spark ge-spark-1">✦</i><i className="ge-spark ge-spark-2">✦</i><i className="ge-spark ge-spark-3">✦</i></div><div className="about-copy"><p className="eyebrow">Meet your esthetician</p><h2>Hi, I’m<br/><em>McKinnley.</em></h2><p>{content.aboutCopy}</p><a className="text-link" href="/about">More about McKinnley →</a></div></section>
    ...
    <section className="section shell"><div className="section-heading"><div><p className="eyebrow">Brows &amp; lashes</p><h2>Framed to <em>perfection.</em></h2></div><a className="text-link" href="/services">See all brow &amp; lash services &rarr;</a></div><div className="service-grid">{browLashServices.map((service, index) => <ServiceCard key={service.id} service={service} index={index}/>)}</div></section>
