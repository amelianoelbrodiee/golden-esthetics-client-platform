...
const labels = ["Your goals", "Your skin", "Your routine", "A final note", "Photo optional"];
...
export default function ConsultationWizard() {
  ...
  return <div className="consultation-shell"><div className="consultation-progress"><div><span>Step {step + 1} of 5</span><strong>{labels[step]}</strong></div><div className="progress-track"><i style={{ width: `${(step + 1) * 20}%` }} /></div></div><div className="consultation-card">
    {step === 0 && <><p className="eyebrow">Let’s begin</p><h2>What are your main skin goals?</h2><p>Choose all that sound like you. There are no wrong answers.</p><ToggleList options={skinGoals} selected={answers.goals} onChange={v => update("goals", v)} /></>}
    ...
    {step === 3 && <><p className="eyebrow">Almost there</p><h2>Anything else you want McKinnley to know?</h2><p>Optional. Please don’t include medical records or sensitive personal information.</p><label className="field-label">Your note<textarea value={answers.notes} onChange={e => update("notes", e.target.value)} placeholder="Your goals or preferences…" maxLength={750} /></label><label className="consent-box insight-consent"><input type="checkbox" checked={shareInsights} onChange={e => setShareInsights(e.target.checked)} /><span><strong>Help Golden Esthetics improve recommendations.</strong> Share an anonymous summary of my selected goals, skin type, sensitivity, recommendation, and whether I used a photo. My note and photo will not be included.</span></label><div className="privacy-note"><strong>Cosmetic guidance, not a diagnosis</strong><p>Your result helps you navigate Golden Esthetics services. It does not diagnose conditions or replace medical care.</p></div></>}
