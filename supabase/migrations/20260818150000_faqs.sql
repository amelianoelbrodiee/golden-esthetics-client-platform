...
-- Written to be idempotent so it applies cleanly even if the table was created
-- by hand first (create-if-not-exists, policies re-created, seed only when empty).
...
create index if not exists faqs_published_order_idx on public.faqs(published, sort_order, created_at);
...
-- Seed the current 8 FAQs, but only if the table is still empty, so this is
-- safe to re-run and won't duplicate anything McKinnley has already added.
insert into public.faqs (question, answer, sort_order, published)
select v.question, v.answer, v.sort_order, v.published
from (values
